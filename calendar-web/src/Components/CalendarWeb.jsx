// Updated CalenderWeb with add/edit/delete functionality
...
const CalenderWeb = () => {
  ...
  const renderDays = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      daysArray.push(<span key={`empty-${i}`} />);
    }
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      daysArray.push(
        <span key={i} className={isToday ? 'current-day' : ''} onClick={() => handleDayClick(i)}>
          {i}
        </span>
      );
    }
      ...
      {showEventPopup && (
        <div className="event-popup">
          <div className="time-input">
            ...
            <input type="number" name="hours" min={0} max={23} value={hours} onChange={(e) => setHours(e.target.value)} className="hours" placeholder="HH" />
            <input type="number" name="minutes" min={0} max={59} value={minutes} onChange={(e) => setMinutes(e.target.value)} className="minutes" placeholder="MM" />
          </div>
          <textarea placeholder="Enter Event Text (Max 60 char)" value={eventText} onChange={(e) => setEventText(e.target.value)} maxLength={60} />
        ...
        {events.map((event, index) => (
          <div className="event" key={index}>
            ...
            <div className="event-text">
              <div className="event-line" style={{ width: '5px', height: '100%', backgroundColor: 'white', marginRight: '1rem', display: 'inline-block' }} />
              {event.text}
            </div>
